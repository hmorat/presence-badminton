...
function App() {
  ...
  return (
    <div style={{ padding: '15px', maxWidth: '500px', margin: 'auto', fontFamily: 'Arial' }}>
      ...
      <button onClick={exportGlobal} style={{ width: '100%', padding: '12px', marginBottom: '20px', backgroundColor: '#f0f0f0', border: '1px solid #ccc', borderRadius: '8px', fontWeight: 'bold' }}>
          ...
          <div style={{ border: '1px solid #eee', borderRadius: '10px', marginBottom: '20px' }}>
            {joueurs.length > 0 ? joueurs.map(j => (
              <div key={j.licence} onClick={() => setPresences({...presences, [j.licence]: !presences[j.licence]})} 
                   style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', borderBottom: '1px solid #eee', backgroundColor: presences[j.licence] ? '#e8f5e9' : 'transparent' }}>
                <span>{j.nom} {j.prenom}</span>
                <input type="checkbox" checked={presences[j.licence] || false} readOnly />
              </div>
            )) : <p style={{textAlign:'center', padding:'20px'}}>Chargement des joueurs...</p>}
          </div>
          <button onClick={sauvegarder} style={{ width: '100%', padding: '15px', backgroundColor: '#2e7d32', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold' }}>
            💾 ENREGISTRER
          </button>
        </>
      )}
